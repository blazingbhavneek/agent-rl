...
timeout=120,
）
for_f in test_dir.glob("test_*.gcov"):
try:
_f.unlink(）
except Exception:
pass
...
@@-1352,6+1341,7@@
attempt += 1
def insert_stub_into_test_file(test_file: Path，func_name: str,body: s
bool:
...
@@-1729,7+1724,6@@
return{
def_normalize_simple_judge_verdict(
3140
```
...
```diff
returnunit_dir/"agent_history"/"good_cunit_backups"/
_safe_filename(fid)
def_best_backup_meta_path(unit_dir:Path,func:dict)->Path:
@@-2016,11+2010,13@@
...
for keyin["percent","coverage","pct","line_percent"]:
...
uses wrappers/stubs from the master test or _stub_gen as neded
* PLACEHOLDER ONLY.
*Target:
{func_id}
* This file must be completely rewritten by the unit-test agent.
...
unit_test_file:Path,
）->None:
""Generate Makefile foraunit test dir，inheriting the master test
Makefile."""
+ ""Generate Makefile foraunit test dir，inheriting the master test
Makefile.
Important behavior:
- If the unit test includes the production .c file directly, the prodt
code is compiled as part of TEST_SRCS.In that mode，do not build/li
prod_under_test.o，otherwise gcov may report coverage from an unexec
```
...
stub_srcs_list = [path for path,_in _stub_srcs_relative(paths["test_dir"],
unit_dir)]
stub_srcs_list=[
path for path，_in _stub_srcs_relative(paths["test_dir"],unit_dir)
+
...
test_text_norm,
flags=re.MULTILINE,
include_paths_norm=[p.replace("\\","/"）forpin include_paths]
production_included_in_test=False
forinc in include_paths_norm:
...
production_included_in_test= True
break
# Suffix match,e.g."../../../../src/dio10od/dio100d.c".
ifinc.endswith("/"+ prod_src_basename):
...
orsource_file_abs_norm in test_text_norm
orf'/{prod_src_basename}"'in test_text_norm
orf'"{prod_src_basename}"'in test_text_norm
orf'<{prod_src_basename}>'in test_text_norm
ifproduction_included_in_test:
...
-$(PROD_OBJ):$(PROD_SRC)
-\t$(CC)$(CPPFLAGS) $(CFLAGS_LINUX) $(CFLAGS) $(INCLUDE) $(COVERAGE_FLAGS)-
Dmain={entry_sym} -c $(PROD_SRC)-0 $(PROD_OBJ)
...
\t@ls -la >>$(TEST_REPORT_FILE)2>&1 |l true
\t@found=0；\
-\tfor f in *.gcno;do \
...
def_generate_unit_test_for_func(
cfg:PipelineConfig，
paths:dict,
@@-2927,6+3028,7@@
...
@@-2982,6+3084,7@@
def_safe_build_diag(make_res:dict）->str:
parts:list[str]=[]
+
try:
...
+ "make_res contained no readable output and no log files were
found."
+ ）
return "\n".join(parts)
@@-3113,61+3220,81@@
...
+Do not remove production-source wiring or replace it with an empty compile-onl
harness just to make the build pass.
+FILES YOU MAY EDIT
...
+
+#define main dio1ood_entry_main
...
+9.A compiling test is not enough. The generated CUnit executable must actually run the registered tests and execute the real production target function. + Always call Cu\_initialize\_registry() before CU\_add\_suite(),register tests with

CU\_add\_test(),call Cu\_basic\_run\_tests()，and verify the target function is called at least once in gcov.If gcov says "function <target> called 0" or the target linesare still"###",the testisinvalid evenif make succeeded.

the Makefile does not also compile/link that same production .c separatelv.
...
）
source_file_abs=_resolve_source_file(cfg, func["source_file"])
funcforprompt={**func，"sourcefile:str(sourcefieb)}
@@-3285,9+3448,9@@
...
the same production .c as a separate object in the Makefile，because that
can
...
-If the target calls exit()，pmf_exit()，abort()，longjmp-like framework
exits,
+ orsimilar terminating functions,add properlinker wrappers such as:
+ -Wl,--wrap=pmf_exit
+ -Wl,--wrap=exit
+ - Implement _wrap_<symbol>(） in the test file so the test can continue.
...
- If production calls pmf_rmtimer(),implement _wrap_pmf_rmtimer().
- Make sure the Makefile contains the matching -Wl,--wrap=<symbol> flag.
-If a function is a macro that expands to another symbol,wrap or stub the
expanded symbol，not only the macro name.
...
- Run gcov on the .gcno file for the object that actually contains the
included
...
run_agent(
@@-3373,29 +3624,32 @@
Rewrite .the unit test file from scratch.Do not preserve placeholc
...
Coverage object:
{cov_text}
The next attempt should make the test execute lines {func.get("start_line")}-
{func.get("end_line")}
18/19
```
