...
describe("AgentRuntime", () => {
	...
	it("persists model finish metadata onto the assistant message", async () => {
		const rawHttpTrace = {
			rawHttpTrace: {
				version: 1,
		...
		const model = new ScriptedModel([
			() => [
				{ type: "text-delta", text: "hello" },
				{ type: "finish", reason: "stop", metadata: rawHttpTrace },
			],
		]);
		const runtime = new AgentRuntime({ model });
		...
		expect(assistantMessage?.metadata).toMatchObject(rawHttpTrace);
	});
