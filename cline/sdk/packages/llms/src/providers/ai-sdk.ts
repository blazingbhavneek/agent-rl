...
type RawHttpTraceMetadata = {
	rawHttpTrace: {
		version: 1;
		providerId: string;
		modelId: string;
		interactions: RawHttpTraceInteraction[];
	};
};
...
function shouldCaptureRawHttpTrace(
	metadata: GatewayStreamRequest["metadata"],
): boolean {
	...
	return (
		metadata.captureRawHttpTrace === true ||
		metadata.captureRawRequestResponse === true ||
		metadata.captureExactRequestResponse === true
	);
}
...
function shouldDisableStreaming(
	...
	return (
		metadata.disableStreaming === true ||
		metadata.disableStream === true ||
		metadata.captureFinalResponseOnly === true
	);
}
...
function headersToObject(
	...
	new Headers(headers).forEach((value, key) => {
		next[key] = shouldRedactHeader(key) ? "[REDACTED]" : value;
	});
	return next;
}
...
async function bodyInitToText(
	...
	if (body instanceof FormData) {
		const serialized = await Promise.all(
			Array.from(body.entries()).map(async ([key, value]) => {
				if (typeof value === "string") {
					return [key, value] as const;
				}
				return [key, await value.text()] as const;
			}),
		);
...
function createRawHttpTraceCollector(
	request: GatewayStreamRequest,
	baseFetch: typeof fetch,
): {
	fetch: typeof fetch;
	finalize: () => Promise<RawHttpTraceMetadata | undefined>;
} {
	const interactions: Array<Promise<RawHttpTraceInteraction>> = [];
	...
	const tracedFetch: typeof fetch = async (input, init) => {
		...
		const requestBodyPromise = captureRequestBody(input, init);
		...
		finalize: async () => {
			...
			return {
				rawHttpTrace: {
					...
					modelId: request.modelId,
					interactions: await Promise.all(interactions),
				},
			};
		},
	};
...
async function* emitAiSdkEvents(
	...
	getRawHttpTraceMetadata?: () => Promise<RawHttpTraceMetadata | undefined>,
	...
	yield {
		...
		error: streamError,
		metadata: getRawHttpTraceMetadata
			? await getRawHttpTraceMetadata()
			: undefined,
	};
}

function buildGenerateTextUsageValue(result: {
	usage?: unknown;
...
async function* emitAiSdkNonStreamingEvents(
	...
	for (const part of result.content) {
		...
		if (
			...
			yield {
				type: "reasoning-delta",
				text: part.text,
				metadata: extractGoogleThoughtMetadata(
					part as unknown as AiSdkStreamPart,
				),
			};
			continue;
		}
		...
		if (part.type === "tool-call") {
			...
			yield {
				type: "tool-call-delta",
				toolCallId: part.toolCallId ?? `tool_${nanoid()}`,
		...
		if (part.type === "tool-error") {
			...
			yield {
				...
				inputText,
				metadata: buildRecoverableToolErrorMetadata({
					part: part as unknown as AiSdkStreamPart,
					errorMessage,
					request,
					context,
					toolName,
	...
	const usageValue = buildGenerateTextUsageValue(result);
	if (usageValue) {
		yield {
			type: "usage",
			usage: normalizeUsage(usageValue, result.providerMetadata, pricingValue),
		};
	}
...
function createAiSdkProvider(kind: ProviderModuleKind): GatewayProviderFactory {
	return async (config) => ({
		async *stream(request, context) {
			...
			try {
				const rawHttpTrace = shouldCaptureRawHttpTrace(request.metadata)
					? createRawHttpTraceCollector(request, ensureFetch(config.fetch))
					: undefined;
				const disableStreaming = shouldDisableStreaming(request.metadata);
				const provider = await createProviderModule(
					kind,
					rawHttpTrace ? { ...config, fetch: rawHttpTrace.fetch } : config,
					context,
				);
				...
				if (disableStreaming) {
					...
					yield* emitAiSdkNonStreamingEvents(
						result,
						request,
						context,
						context.model.metadata?.pricing,
						rawHttpTrace ? () => rawHttpTrace.finalize() : undefined,
					);
					return;
				}
				stream = streamText({
