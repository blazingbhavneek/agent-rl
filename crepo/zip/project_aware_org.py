# =============================================================================
# REGION 01 - IMPORTS, GLOBALS, AND CALL-GRAPH DISPLAY HELPERS
# =============================================================================
#
# Context:
#   This region is copied from the original project_aware.py. It contains the
#   imports, global color/state constants, Tree-sitter function-call extraction,
#   function detection helpers, and call-path tree printing helpers.
#
# Copy-paste note:
#   This region is intentionally under 600 lines so it can be pasted into a chat
#   UI with enough surrounding context for small edits.
#
# Region map:
#   REGION 01 - imports/globals/helper tree utilities. You are here.
#   REGION 02 - llm_calls prompts and single LLM call wrapper.
#   REGION 03 - run_with_retry child-process timeout wrapper.
#   REGION 04 - first half of make_llm_calls_for_function.
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration.
#   REGION 07 - __main__ manual batch runner.
#
# Ask-next hint:
#   If a chat model needs the next runtime step after helpers, ask for REGION 02.
#
...
# =============================================================================
# REGION 02 - LLM PROMPTS AND SINGLE LLM CALL WRAPPER
# =============================================================================
#
# Context:
#   This region builds the original static-tracing prompts and calls OllamaClient.
#   It returns exactly the original shape: (validated_answer_model, stats_model).
#   Keep this contract if you want project_aware.py-compatible behavior.
#
# Copy-paste note:
#   This region is intentionally under 600 lines.
#
# Region map:
#   REGION 01 - imports/globals/helper tree utilities.
#   REGION 02 - llm_calls prompts and single LLM call wrapper. You are here.
#   REGION 03 - run_with_retry child-process timeout wrapper.
#   REGION 04 - first half of make_llm_calls_for_function.
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration.
...
#
# Ask-next hint:
#   If editing how LLM calls are retried/timed out, ask for REGION 03. If editing
#   how llm_calls is used per path, ask for REGION 04 and REGION 05.
#
...
# =============================================================================
# REGION 03 - PROCESS-SAFE RETRY UTILITY
# =============================================================================
#
# Context:
#   This is the original retry wrapper. It runs the passed function in a child
#   process, kills timed-out attempts, and returns the child result.
#
# Copy-paste note:
#   This region is intentionally under 600 lines.
#
...
#   REGION 01 - imports/globals/helper tree utilities.
#   REGION 02 - llm_calls prompts and single LLM call wrapper.
#   REGION 03 - run_with_retry child-process timeout wrapper. You are here.
#   REGION 04 - first half of make_llm_calls_for_function.
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration.
#   REGION 07 - __main__ manual batch runner.
#
# Ask-next hint:
#   If editing callers of run_with_retry, ask for REGION 04 and REGION 05.
#
...
# =============================================================================
# REGION 04 - PER-FUNCTION PROCESSOR: SETUP, CALL GRAPH, DEPENDENT-FUNCTION PATHS
# =============================================================================
#
# Context:
#   This starts the original make_llm_calls_for_function implementation. It:
#     - initializes state/token counters,
#     - builds call graph paths,
#     - parses context,
#     - handles the dependent-function branch.
#
#   The direct/no-dependent-function branch continues in REGION 05.
#
# Copy-paste note:
#   This region is intentionally under 600 lines, but it is the first half of a
#   large function. If editing this region in isolation, keep REGION 05 nearby.
#
# Region map:
#   REGION 01 - imports/globals/helper tree utilities.
#   REGION 02 - llm_calls prompts and single LLM call wrapper.
#   REGION 03 - run_with_retry child-process timeout wrapper.
#   REGION 04 - first half of make_llm_calls_for_function. You are here.
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration.
#   REGION 07 - __main__ manual batch runner.
#
# Ask-next hint:
#   If editing this region, ask for REGION 05 too because it closes the same
#   make_llm_calls_for_function body. For the caller, ask for REGION 06.
#
...
def make_llm_calls_for_function(
    ...
    if len(path_nodes) > 0:
        ...
        # =============================================================================
        # REGION 05 - PER-FUNCTION PROCESSOR: DIRECT PATHS AND FINAL ROW BUILDING
        # =============================================================================
        #
        # Context:
        #   This is the no-dependent-function branch of make_llm_calls_for_function.
        #   It keeps the original behavior:
        #     - skip LLM for pmf functions with no target args,
        #     - call the LLM once per path,
        #     - parse output/call_number,
        #     - write CSV rows and stats JSON.
        #
        # Copy-paste note:
        #   This region is intentionally under 600 lines and is indented because it
        #   lives inside make_llm_calls_for_function.
        #
        # Region map:
        #   REGION 01 - imports/globals/helper tree utilities.
        #   REGION 02 - llm_calls prompts and single LLM call wrapper.
        #   REGION 03 - run_with_retry child-process timeout wrapper.
        #   REGION 04 - first half of make_llm_calls_for_function.
        #   REGION 05 - second half/direct branch of make_llm_calls_for_function. You are here.
        #   REGION 06 - trace_variable project-level orchestration.
        #   REGION 07 - __main__ manual batch runner.
        #
        # Ask-next hint:
        #   If editing this region, ask for REGION 04 too because both are one
        #   function. If editing project setup/call order, ask for REGION 06.
        #
...
# =============================================================================
# REGION 06 - PROJECT-LEVEL ORCHESTRATION
# =============================================================================
#
# Context:
#   trace_variable(project_path) is the original project-level spine. It loads or
#   builds project structure, preprocesses files, identifies trace targets, then
#   calls make_llm_calls_for_function for every detected function.
#
# Copy-paste note:
#   This region is intentionally under 600 lines.
#
...
#   REGION 04 - first half of make_llm_calls_for_function.
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration. You are here.
#   REGION 07 - __main__ manual batch runner.
#
# Ask-next hint:
#   If editing per-function behavior called from here, ask for REGION 04 and
#   REGION 05. If editing manual project selection, ask for REGION 07.
#
...
# =============================================================================
# REGION 07 - SCRIPT ENTRYPOINT / MANUAL BATCH RUNNER
# =============================================================================
#
# Context:
#   This is the original __main__ block. It sets logging paths, picks or hardcodes
#   source project groups, initializes State, and runs trace_variable.
#
# Copy-paste note:
#   This region is intentionally under 600 lines.
#
...
#   REGION 05 - second half/direct branch of make_llm_calls_for_function.
#   REGION 06 - trace_variable project-level orchestration.
#   REGION 07 - __main__ manual batch runner. You are here.
#
# Ask-next hint:
#   If editing the actual tracer invoked by this runner, ask for REGION 06. If
#   editing per-path behavior, ask for REGION 04 and REGION 05.
#
